...
func (handler *SQLHandler) GetDynoByNickname(nickname string) (Animal, error) {
	// ? for mysql or sqlite and it used to be $1 for pq, so it is better to just string instead of parametered query
...
func (handler *SQLHandler) UpdateAnimal(a Animal, nname string) error {
	_, err := handler.Exec(fmt.Sprintf("Update Animals set Animal_type = '%s', nickname = '%s', nickname = '%s', zone = %d, age = %d where nickname = '%s'", a.AnimalType, a.Nickname, a.Zone, a.Age, nname))
	return err
}
